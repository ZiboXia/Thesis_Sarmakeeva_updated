...
% Some abstract text
\abstract{
Each year, landslides cause more than 5000 deaths. With increasing flood frequency and severity exacerbated by drought and global warming, understanding and predicting landslides involving granular media, water, and air is crucial. Our research aims to study the interaction of particles, rocks, and boulders with water.

For simulations, we use CFD methods combined with the Discrete Element Method (DEM) to represent solid bodies of arbitrary shapes. We developed a force model based on the open-source code CFDEMcoupling\cite{kloss2012models} to improve the simulation model. We use a resolved CFD-DEM approach, where the solid body takes more than a computational cell. This simulation helps to track interactions between fluids and solids, such as the movement of soil or rocks falling into water.

In our study, the fluid and solid phases are modeled using the Eulerian framework for fluids and the Lagrangian framework for solids. The Immersed Boundary Method is employed to integrate the solid component into the CFD mesh, allowing the projection of a solid body into the fluid domain. We utilize the sharp interface VOF method to simulate a free surface. Our validation and verification processes confirm the solver's functionality as anticipated.

We conducted extensive solver testing, initially with a bouncing body simulation that demonstrated stable results. Further, we explored interactions between multiple spherical bodies and fluid using the developed solver. This demonstrated its capability to effectively manage complex interactions between solid and liquid phases and the dynamics of multiple solid bodies. Such capabilities are crucial for accurately simulating real-world phenomena like landslides, sediment transport, and other particulate flow issues.

Our solver is adept at handling geometric complexities. It can accurately track the motion and interaction of non-uniformly shaped bodies within the fluid. This process involves dealing with complex boundary conditions and potentially non-linear material behavior. Additionally, our results indicate the solver's efficiency in parallel computations, evidenced by reduced computation times with increased processor counts.

For broader accessibility and reproducibility, we have made the solver's documentation available online. This includes comprehensive guidance and a container application to facilitate its use. 
